...
async function init() {
  ...
  const roomMat = new THREE.MeshBasicMaterial({ color: 0xff0000, wireframe: true });
  map.mapData.rooms.forEach(r => {
    const room = new THREE.Mesh(new THREE.BoxGeometry(20, 10, 20), roomMat);
    room.position.set(r.x, 5, r.z);
    scene.add(room);
  });
...
socket.on("receive-chat", d => {
  ...
  log.innerHTML += `<div><b>Player:</b> ${d.msg}</div>`;
